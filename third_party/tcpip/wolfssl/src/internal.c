                          ...
                          byte type, word32 size, word32 totalSz)
{
    ...
    //SYS_CONSOLE_MESSAGE("###DoHandShakeMsgType###");
#ifdef WOLFSSL_TLS13
    ...
    //SYS_CONSOLE_MESSAGE("***SanityCheckMsgReceived...OK***");
    ...
    if (ssl->options.handShakeState == HANDSHAKE_DONE && type != hello_request){
        ...
        //SYS_CONSOLE_MESSAGE("***HandShake message after handshake complete***");
        return OUT_OF_ORDER_E;
    }
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.dtls == 0 &&
        ...
        SendAlert(ssl, alert_fatal, unexpected_message);
        //SYS_CONSOLE_MESSAGE("***First server message not server hello***");
        return OUT_OF_ORDER_E;
    }
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.dtls &&
        ...
        SendAlert(ssl, alert_fatal, unexpected_message);
        //SYS_CONSOLE_MESSAGE("***Server hello done received before server hello in DTLS***");
        return OUT_OF_ORDER_E;
    }
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END &&
        ...
        SendAlert(ssl, alert_fatal, unexpected_message);
        //SYS_CONSOLE_MESSAGE("***First client message not client hello***");
        return OUT_OF_ORDER_E;
    }
    ...
    if (type != hello_request &&
        ...
        if (ret != 0) 
        {
            //SYS_CONSOLE_MESSAGE("***HashInput something wrong!***");
            return ret;
        }
    }
    ...
    default:
        ...
        //SYS_CONSOLE_MESSAGE("***Unknown handshake message type...***");
        ret = UNKNOWN_HANDSHAKE_TYPE;
        break;
    }
    if (ret == 0 && expectedIdx != *inOutIdx) {
        WOLFSSL_MSG("Extra data in handshake message");
        //SYS_CONSOLE_MESSAGE("***Extra data in handshake message...***");
        if (!ssl->options.dtls)
    ...
    //SYS_CONSOLE_MESSAGE("***DoHandShakeMsgType LEAVE...***");
    return ret;
}
                          ...
                          word32 totalSz)
{
    ...
    if (ssl->arrays == NULL) {
        ...
        {
            //SYS_CONSOLE_MESSAGE("***GetHandShakeHeader...ErrorParse_***");
            return PARSE_ERROR;
        }
        ssl->options.handShakeState = type;
        //SYS_CONSOLE_MESSAGE("***DoHandShakeMsgType...A***");
    ...
    if (ssl->arrays->pendingMsgSz == 0) {
        ...
        if (GetHandShakeHeader(ssl,input, inOutIdx, &type, &size, totalSz) != 0)
        {
            //SYS_CONSOLE_MESSAGE("***GetHandShakeHeader...ErrorParse***");
            return PARSE_ERROR;
        }
        ...
        if (size > MAX_HANDSHAKE_SZ) {
            ...
            //SYS_CONSOLE_MESSAGE("GetHandShakeHeader...ErrorSize");
        ...
        //SYS_CONSOLE_MESSAGE("***DoHandShakeMsgType...B***");
        ...
        if (inputLength + ssl->arrays->pendingMsgOffset
            ...
            //SYS_CONSOLE_MESSAGE("***GetHandShakeHeader...ErrorBuffer***");
            return BUFFER_ERROR;
        }
        ...
        *inOutIdx += inputLength;
        ...
        {
            word32 idx = 0;
            //SYS_CONSOLE_MESSAGE("***DoHandShakeMsgType...C***");
...
int ProcessReply(WOLFSSL* ssl)
{
    ...
    for (;;) {
         ...
         * old client hello */
        case doProcessInit:
            if(SYS_TMR_TickCountGet() - startTick >= SYS_TMR_TickCounterFrequencyGet() * 0.15)
            {
                //SYS_CONSOLE_MESSAGE("###doProcessInit###");
        ...
        case getRecordLayerHeader:
            //SYS_CONSOLE_MESSAGE("###getRecordLayerHeader###");
        ...
        case getData:
            if(SYS_TMR_TickCountGet() - startTick >= SYS_TMR_TickCounterFrequencyGet() * 0.25)
            {
                //SYS_CONSOLE_MESSAGE("###getData###");
                startTick = SYS_TMR_TickCountGet();
            }
            /* get sz bytes or return error */
            if (!ssl->options.dtls) {
        ...
        /* decrypt message */
        case decryptMessage:
            //SYS_CONSOLE_MESSAGE("###decryptMessage###");
        ...
        case verifyMessage:
                //SYS_CONSOLE_MESSAGE("###verifyMessage###");
#if !defined(WOLFSSL_TLS13) || defined(WOLFSSL_TLS13_DRAFT_18)
            if (IsEncryptionOn(ssl, 0) && ssl->keys.decryptedCur == 0)
#else
            if (IsEncryptionOn(ssl, 0) && ssl->keys.decryptedCur == 0 &&
        ...
        case runProcessingOneMessage:
            //SYS_CONSOLE_MESSAGE("###runProcessingOneMessage###");
        #ifdef WOLFSSL_DTLS
            if (IsDtlsNotSctpMode(ssl)) {
            ...
            switch (ssl->curRL.type) {
                ...
                case change_cipher_spec:
                    //SYS_CONSOLE_MESSAGE("---change_cipher_spec---");
                ...
                case application_data:
                    //SYS_CONSOLE_MESSAGE("---application_data---");
