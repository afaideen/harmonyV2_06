...
// *****************************************************************************
// *****************************************************************************
//#define DRV_SST25_BUFFER_OBJ_NUMBER                       13
...
const uint32_t gSstFlashIdSizeTable [DRV_SST25_NUM_DEVICE_SUPPORTED][2] = {
    ...
    {0x6E, 0x80000},  /* SST25PF040C - 4 MBit */
    {0x8E, 0x100000}, /* SST25VF080B - 8 MBit */
    {0x86, 0x100000}, /* SST25WF080B - 8 MBit */
...
//    .sectorWrite        = DRV_SST25_BlockWrite,
...
)
{
    ...
    else if (deviceId == 0x41)
    {
        dObj->mediaGeometryTable[1].blockSize = 1;
        dObj->mediaGeometryTable[1].numBlocks = flashSize;
//        dObj->mediaGeometryTable[1].numBlocks = 2*1024*1024;	//  2M or 2,097,152 to be exact!
        dObj->flashFunctions.write = DRV_SST25_WritePageProgram;
...
)
{
    ...
    return clientObj;
//    return dObj;
//    return (DRV_SST25_OBJ *) clientObj->driverObj;
}
...
//    sst25Init->csPort = DRV_SST25_CHIP_SELECT_PORT_CHANNEL_IDX0;
//    sst25Init->csPin = DRV_SST25_CHIP_SELECT_PORT_BIT_POS_IDX0;
...
//    dObj->csPort =  DRV_SST25_CHIP_SELECT_PORT_CHANNEL_IDX0;
    dObj->csPin = sst25Init->csPin;
//    dObj->csPin = DRV_SST25_CHIP_SELECT_PORT_BIT_POS_IDX0;
...
//    dObj->spiClientData.baudRate = 0;//DRV_SPI_BAUD_RATE_IDX0;//0;
    dObj->spiClientData.baudRate = DRV_SPI_BAUD_RATE_IDX0;//0;
...
)
{
    ...
    *tempHandle1 = DRV_SST25_BLOCK_COMMAND_HANDLE_INVALID;
    ...
    {
        SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "DRV_SST25_BlockRead(): Invalid parameters.\n");
        return;
    }
...
)
{
    ...
    *tempHandle1 = DRV_SST25_BLOCK_COMMAND_HANDLE_INVALID;
    ...
    {
        SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "DRV_SST25_BlockWrite(): Invalid parameters.\n");
        return;
    }
...
//    if ( nBlock == 0) 
    {
        SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "DRV_SST25_BlockErase(): Invalid parameters.\n");
        return;
    }
...
)
{
    ...
    *tempHandle1 = DRV_SST25_BLOCK_COMMAND_HANDLE_INVALID;
    ...
    if ((sourceBuffer == NULL) || (nBlock == 0) || ((blockStart + nBlock) > dObj->mediaGeometryTable[DRV_SST25_GEOMETRY_TABLE_WRITE_ENTRY].numBlocks))
    {
        SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "DRV_SST25_BlockEraseWrite(): Invalid parameters.\n");
        return;
    }
...
)
{
    ...
    if(object >= DRV_SST25_INSTANCES_NUMBER)
    {
        SYS_DEBUG(SYS_ERROR_DEBUG, "DRV_SST25: Invalid module object \n");
        return;
    }
