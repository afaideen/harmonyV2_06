...
//} sqi_dma_desc_t;
//
//sqi_dma_desc_t __attribute__((coherent)) sqiCmdDesc[5];
//sqi_dma_desc_t __attribute__((coherent)) sqiBufDesc[32];
//
////uint8_t __attribute__((coherent)) statusRegVal;
//
////uint8_t __attribute__((coherent)) jedecID[4];
//
////uint8_t __attribute__((coherent)) sqi_cmd_jedec[2];
//uint8_t __attribute__((coherent)) sqi_cmd_eqio;
//uint8_t __attribute__((coherent)) sqi_cmd_rsten;
//uint8_t __attribute__((coherent)) sqi_cmd_rst;
//uint8_t __attribute__((coherent)) sqi_cmd_wren;
//uint8_t __attribute__((coherent)) sqi_cmd_rdsr[2];
//uint8_t __attribute__((coherent)) sqi_cmd_ce;
//uint8_t __attribute__((coherent)) sqi_cmd_se[4];
//uint8_t __attribute__((coherent)) sqi_cmd_be[4];
//uint8_t __attribute__((coherent)) sqi_cmd_pp[4];
//uint8_t __attribute__((coherent)) sqi_cmd_hsr[7];
//uint8_t __attribute__((coherent)) sqi_cmd_ULBPR;
//uint8_t __attribute__((coherent)) sqi_cmd_dummy[6];
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
)
{
    ...
    dObj->mediaGeometryTable[0].numBlocks = flashSize;//4194304

    /* Write block size and number of blocks */
    dObj->mediaGeometryTable[1].blockSize = 256;
    dObj->mediaGeometryTable[1].numBlocks = flashSize >> 8;//16384

    /* Erase block size and number of blocks */
    dObj->mediaGeometryTable[2].blockSize = 4096;
    dObj->mediaGeometryTable[2].numBlocks = flashSize >> 12;//1024
        ...
        case DRV_SST26_SUBTASK_RESET_ENABLE_DELAY:
            {
                if (SYS_TMR_DelayStatusGet(dObj->tmrHandle))
                {
                    ...
                    else
                    {
                        ...
                        cmdStatus = DRV_SQI_COMMAND_QUEUED;
                        dObj->tmrHandle = SYS_TMR_DelayMS (1);
                    }
                }
                else
                {
...
//                sprintf(sval, "address: %d data: 0X%X\r\n", frame->address, data);
//                putsUART2(sval);
                Nop();
//                memset (&sval, 0, 256);
//                memcpy(&sval, data, 256);
...
//                putsnUART2(&sval, 256);
                Nop();
                DRV_SQI_TransferFrames (dObj->sqiHandle, &dObj->cmdHandle, frame, 1);
                memcpy(&sval, data, 256);//related to dma operation
//                unsigned long g;
//                for( g = 0; g < 100000; g ++)
//                    Nop();
...
//                sprintf(sval, "srcAddr: 0X%X, blockAddress: %d, bufferOffset: %d,\r\n", &dObj->writePtr[dObj->bufferOffset], dObj->blockAddress, dObj->bufferOffset);
//                putsUART2(sval);
                Nop();
            }
...
//    return status;
//}
//static void DRV_SST26_WriteEnable_(void)
//{
//    sqi_cmd_wren                = DRV_SST26_CMD_WRITE_ENABLE;
//
//    sqiCmdDesc[0].bd_ctrl       = ( SQI_BDCTRL_BUFFLEN_VAL(1) | SQI_LANE_MODE |
...
//    }
//
//    sqi_cmd_ce = DRV_SST26_CMD_CHIP_ERASE;
//
//    return (DRV_SST26_Erase_(&sqi_cmd_ce, 1));
//}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static DRV_SQI_COMMAND_STATUS DRV_SST26_ChipErase_
(
    DRV_SST26_OBJECT *dObj,
    uint8_t *data,
    uint32_t blockAddress,
...
//    unsigned char sval[256];
    ...
    switch (dObj->subState)
    {
        ...
        default:
            {
                DRV_SQI_TransferFrame *frame = dObj->xferFrame;
                frame->instruction = DRV_SST26_CMD_CHIP_ERASE;
                frame->address = blockAddress;
                frame->data = NULL;
                frame->length = 0;
                frame->laneCfg = DRV_SQI_LANE_QUAD_ALL;
        ...
        default:
            {
                ...
                frame->address = blockAddress;
                frame->data = NULL;
                frame->length = 0;  //original
//                frame->length = nBlocks * 4096;
                frame->laneCfg = DRV_SQI_LANE_QUAD_ALL;
                frame->numDummyBytes = 0;
                frame->flags = (DRV_SQI_FLAG_INSTR_ENABLE_MASK | DRV_SQI_FLAG_ADDR_ENABLE_MASK);
                frame->flags |= DRV_SQI_FLAG_SQI_CS_NUMBER(dObj->sqiDevice);
...
static DRV_SQI_COMMAND_STATUS DRV_SST26_HandleChipErase(DRV_SST26_OBJECT *dObj)
{
    DRV_SQI_COMMAND_STATUS cmdStatus = DRV_SQI_COMMAND_ERROR_UNKNOWN;
...
//                dObj->nBlocks = nBlocks;
        ...
        case DRV_SST26_ERASE_CMD:
            {     
                cmdStatus = DRV_SST26_ChipErase_ (dObj, NULL, dObj->blockAddress, 0);    //Chip Erase
                if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
                {
                    dObj->subState = DRV_SST26_SUBTASK_CMD;
                    dObj->eraseState = DRV_SST26_ERASE_CMD_STATUS;
//                    dObj->tmrHandle = SYS_TMR_DelayMS (20);
                    cmdStatus = DRV_SQI_COMMAND_QUEUED;
                }
                break;
            }
...
)
{
    ...
    switch (dObj->eraseState)
    {
        ...
        case DRV_SST26_ERASE_CMD:
            {
                cmdStatus = DRV_SST26_SectorErase (dObj, NULL, dObj->blockAddress, 0);    //original      
//                cmdStatus = DRV_SST26_ChipErase_ (dObj, NULL, dObj->blockAddress, 0);    //Chip Erase
                if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
                {
                    dObj->subState = DRV_SST26_SUBTASK_CMD;
...
//                    sprintf(sval, "blockStart:%d, sectorAddress: %d, source: 0X%X,\r\n", bufferObj->blockStart, dObj->sectorAddress*16, dObj->writePtr);
//                    putsUART2(sval);
...
)
{
    ...
    dObj->numClients = 0;
//    dObj->numClients = 1;
//    dObj->numClients = sst26Init->sqiDevice;
...
//    dObj->blockStartAddress = 0;//original
    dObj->blockStartAddress = DRV_SST26_MEDIA_START_ADDRESS_IDX0;
...
//    uint8_t sval[60];
//    sprintf(sval, "blockStart: %d nBlock: %d\r\n", blockStart, nBlock);
//    putsUART2(sval);
    dObj = clientObj->driverObj;
    if ((sourceBuffer == NULL) || (nBlock == 0) || ((blockStart + nBlock) > dObj->mediaGeometryTable[DRV_SST26_GEOMETRY_TABLE_WRITE_ENTRY].numBlocks))
...
bool erase_chip(void){
    ...
    dObj = &gDrvSST26Obj[0];
    cmdStatus = DRV_SST26_HandleChipErase(dObj);
    if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
    {
        Nop();
        return true;
        ...
        case DRV_SST26_OPEN_SQI:
            {
                ...
                if (dObj->sqiHandle != DRV_HANDLE_INVALID)
                {
                    ...
                    dObj->state = DRV_SST26_RESET_FLASH;    //original
                    dObj->subState = DRV_SST26_SUBTASK_RESET_ENABLE_CMD;  //original
//                    dObj->state = DRV_SST26_ERASE_FLASH;
//                    dObj->subState = DRV_SST26_ERASE_INIT;
...
//                    dObj->tmrHandle = SYS_TMR_DelayMS (2);
        ...
        case DRV_SST26_READ_FLASH_ID:
            {
                ...
                if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
                {
                    ...
                    static unsigned char dummy, devicetype, deviceid, manufacturerid;
                    unsigned char *statusReg, *flashId;
                   
                    flashId = dObj->flashId;
                    val[0] = *flashId++;
                    val[1] = *flashId++;
                    val[2] = *flashId++;
                    val[3] = *flashId;
                    ...
                    statusReg = dObj->statusReg;
                    manufacturerid  = val[0];
                    devicetype      = val[1];
                    deviceid        = val[2];
                    if (deviceid != 0 && deviceid != 0xff){
                        SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "DRV_SST26_READ_FLASH_ID: Flash ID is 0X%X\n", deviceid);
//                        SYS_CONSOLE_PRINT("DRV_SST26_READ_FLASH_ID: Flash ID is %X\n", deviceid);
                    }
                    else{
                        SYS_CONSOLE_MESSAGE("DRV_SST26_READ_FLASH_ID: ID NOT found");
                    }
//                    if (DRV_SST26_UpdateGeometry (dObj, dObj->flashId[2]))
                    if ( DRV_SST26_UpdateGeometry(dObj, deviceid) )
                    {
        ...
        case DRV_SST26_UNLOCK_FLASH:
            {
                ...
                if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
                {
                    ...
                    dObj->state = DRV_SST26_PROCESS_QUEUE;    //original
                    dObj->status = SYS_STATUS_READY;          //original
//                    dObj->state = DRV_SST26_ERASE_FLASH;
//                    dObj->subState = DRV_SST26_ERASE_INIT;
                }
                else if (cmdStatus == DRV_SQI_COMMAND_ERROR_UNKNOWN)
        ...
        case DRV_SST26_ERASE_FLASH:
//            cmdStatus = DRV_SST26_HandleErase(dObj, NULL, dObj->sectorAddress, 4096);//4096);//1);
//            cmdStatus = DRV_SST26_HandleErase(dObj, NULL, 1, 1);//4096);//1);
            cmdStatus = DRV_SST26_HandleChipErase(dObj);
            if (cmdStatus == DRV_SQI_COMMAND_COMPLETED)
            {
