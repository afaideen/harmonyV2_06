...
#else
//    void __longcall__ __attribute__ ((section (".cache_init"))) SwapFlashPanels_(uint8_t btn)
    void __longcall__ SwapFlashPanels_(uint8_t btn)
    {
        PLIB_NVM_MemoryModifyInhibit(NVM_ID_0);
        PLIB_NVM_FlashWriteKeySequence(NVM_ID_0, NVM_PROGRAM_UNLOCK_KEY1);
        PLIB_NVM_FlashWriteKeySequence(NVM_ID_0, NVM_PROGRAM_UNLOCK_KEY2);
    ...
    //    if(PLIB_NVM_BootFlashBank2IsLowerRegion(NVM_ID_0) == true)
        if( btn == 1 )
        {
...
void Bootloader_Tasks ()
{
    ...
    case BOOTLOADER_SEND_RESPONSE:
        ...
        case BOOTLOADER_ENTER_APPLICATION:
            ...
            if ((NVMCON & _NVMCON_PFSWAP_MASK) != _NVMCON_PFSWAP_MASK)
            {
                //running on bank 1, switch to bank 2 then!
                Enter_App(2);
            }
            else
            {
