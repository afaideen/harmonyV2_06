                        ...
                        switch (bootloaderData.data->buffers.buff1[i])
                        {
                            ...
                            case EOT:   // End of transmission
                                ...
                                if (bootloaderData.cmdBufferLength > 2)
                                {
                                    ...
                                    calculated_crc = APP_CalculateCrc(bootloaderData.data->buffers.buff2, bootloaderData.cmdBufferLength-2);
                                    Nop();
    ...
    switch (Cmd)
    {
        ...
        case PROGRAM_FLASH:
            if(APP_ProgramHexRecord(&handle->data->buffers.buff2[1], handle->cmdBufferLength-3) != HEX_REC_NORMAL)
                break;
//            if(bootloader_NvmProgramHexRecord(&handle->data->buffers.buff2[1], handle->cmdBufferLength-3) != HEX_REC_NORMAL)
//                break;
            handle->bufferSize = 1;
            handle->currentState = BOOTLOADER_SEND_RESPONSE;
            break;
...
//           if(appData.fw_bank_running == 1)
//           {
//               //calculate crc on bank 2
//               Address = UPPER_FLASH_APP_BASE_ADDRESS;
//               BSP_LED_1Off();
//           }else
//           {
//               //calculate crc on bank 1
//               Address = LOWER_FLASH_APP_BASE_ADDRESS;
//               BSP_LED_1On();
//           }
