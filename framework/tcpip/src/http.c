...
#if defined(DRV_SST25_CLIENTS_NUMBER)      
        #define MPFS_UPLOAD_WRITE_BUFFER_SIZE       4096//(SYS_FS_MEDIA_MANAGER_BUFFER_SIZE)
#elif defined(DRV_SQI_CLIENTS_NUMBER) 
        #define MPFS_UPLOAD_WRITE_BUFFER_SIZE       4096//256//(256)
#else
...
//#if defined(DRV_SST25_CLIENTS_NUMBER)
//        
//        http_malloc_fnc = malloc(512);
//        http_free_fnc = free(512);
//#else
        http_malloc_fnc = httpInitData->http_malloc_fnc;//original
...
#if defined(DRV_SST25_CLIENTS_NUMBER)        
        #define     SYS_FS_MEDIA_SECTOR_SIZE        4096   
#elif defined(DRV_SQI_CLIENTS_NUMBER)
        #define     SYS_FS_MEDIA_SECTOR_SIZE        MPFS_UPLOAD_WRITE_BUFFER_SIZE//4096//256
#else
...
static HTTP_IO_RESULT TCPIP_HTTP_MPFSUpload(HTTP_CONN* pHttpCon)
{
    ...
    switch(pHttpCon->httpStatus)
    {
        ...
        case HTTP_MPFS_UP:
            SYS_DEBUG_PRINT(SYS_ERROR_INFO, "pHttpCon->httpStatus: HTTP_MPFS_UP\n");
            #if defined(DRV_SST25_CLIENTS_NUMBER)   
                pHttpCon->uploadSectNo = 1;//DRV_SST25_MEDIA_START_ADDRESS_IDX0/4096;
            #elif defined(DRV_SQI_CLIENTS_NUMBER)    
                pHttpCon->uploadSectNo = DRV_SST26_MEDIA_START_ADDRESS_IDX0/4096;
            #else
            pHttpCon->uploadSectNo = 0;
            #endif
            lenA = TCPIP_TCP_ArrayFind(pHttpCon->socket, (const uint8_t*)"\r\n\r\n", 4, 0, 0, false);
            SYS_CONSOLE_PRINT("pHttpCon->httpStatus: HTTP_MPFS_UP, lenA: %d\r\n", lenA);
            if(lenA != 0xffff)
            {// Found it, so remove all data up to and including
                ...
                SYS_CONSOLE_PRINT("nvmBuffer: %s\r\n", nvmBuffer);
                if(memcmp(nvmBuffer, (const void*)MPFS_SIGNATURE, sizeof(MPFS_SIGNATURE)-1) == 0)
                {   // Read as Ver 2.1
                    SYS_CONSOLE_MESSAGE("\"MPFSx01x02\" detected\r\n");
                    // allocate the buffer size as a multiple of sector size
                    #if defined(DRV_SST25_CLIENTS_NUMBER) || defined(DRV_SQI_CLIENTS_NUMBER)       
                        mpfsAllocSize = MPFS_UPLOAD_WRITE_BUFFER_SIZE;
                    #else
                    ...
                    if(pHttpCon->uploadBufferStart == 0)
                    {
                        ...
                        SYS_CONSOLE_PRINT("SYS_FS_Unmount_Wrapper, mpfsAllocSize = %lu\r\npHttpCon->uploadBufferStart = %X\r\n", mpfsAllocSize, pHttpCon->uploadBufferStart);
                    }

                    if(pHttpCon->uploadBufferStart != 0)
                    {
                        SYS_CONSOLE_PRINT("uploadBufferStart = %X\r\n", pHttpCon->uploadBufferStart);
                        pHttpCon->uploadBufferEnd = pHttpCon->uploadBufferStart + mpfsAllocSize;
                        SYS_CONSOLE_PRINT("uploadBufferEnd = %X\r\n", pHttpCon->uploadBufferEnd);
            ...
            {// Otherwise, remove as much as possible
                ...
                SYS_CONSOLE_PRINT("pHttpCon->byteCount: %lu, lenA: %d\r\n", pHttpCon->byteCount, lenA);
                pHttpCon->byteCount -= lenA;
                SYS_CONSOLE_PRINT("pHttpCon->byteCount: %lu\r\n", pHttpCon->byteCount);
            }
            break;

        case HTTP_MPFS_ERROR:
            SYS_CONSOLE_MESSAGE("HTTP_MPFS_ERROR\r\n");
            if(pHttpCon->uploadBufferStart != 0)
            {
        ...
        case HTTP_MPFS_OK:
 //           SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "HTTP_MPFS_OK\n");
            lenA = TCPIP_TCP_GetIsReady(pHttpCon->socket);
...
//                SYS_CONSOLE_PRINT("lenA = %d\r\n", lenA);
 ...
 //               SYS_CONSOLE_PRINT("pHttpCon->uploadBufferEnd - pHttpCon->uploadBufferCurr = %d\r\n", lenA);
            }

            if(lenA > pHttpCon->byteCount)
            {
                lenA = pHttpCon->byteCount;
//                SYS_CONSOLE_PRINT("pHttpCon->byteCount = %d##", lenA);
            }

            if(lenA == 0 )
            {
//                SYS_CONSOLE_MESSAGE("HTTP_IO_WAITING#");
                return HTTP_IO_WAITING;
            }

            lenB = TCPIP_TCP_ArrayGet(pHttpCon->socket, pHttpCon->uploadBufferCurr, lenA);
            pHttpCon->uploadBufferCurr += lenB; //SYS_CONSOLE_PRINT("uploadBufferCurr = %X#", pHttpCon->uploadBufferCurr);
            pHttpCon->byteCount -= lenB;        //SYS_CONSOLE_PRINT("byteCount = %d # lenB = %d\r\n", pHttpCon->byteCount, lenB);
...
//                SYS_CONSOLE_MESSAGE("HTTP_IO_WAITING#");
            ...
            Nop();
            SYS_CONSOLE_PRINT("SectorWrite(%d,%d,%X, %lu, %d, %d, %d)\r\n", MPFS_UPLOAD_DISK_NO, pHttpCon->uploadSectNo, pHttpCon->uploadBufferStart, nSectors, lenA, lenB, pHttpCon->byteCount);
//            SYS_DEBUG_PRINT(SYS_ERROR_DEBUG, "SectorWrite(%d,%d,%X, %lu)\r\n", MPFS_UPLOAD_DISK_NO, pHttpCon->uploadSectNo, pHttpCon->uploadBufferStart, nSectors);
#if defined(EAGLEEYE2014)
            #if defined(TCPIP_IF_MRF24W) || defined(TCPIP_IF_MRF24WN) 
            WF_INT_IE = 0;
            #endif
#endif
            pHttpCon->uploadBuffHandle = SYS_FS_MEDIA_MANAGER_SectorWrite(MPFS_UPLOAD_DISK_NO, pHttpCon->uploadSectNo , pHttpCon->uploadBufferStart, nSectors);
#if defined(EAGLEEYE2014)
            #if defined(TCPIP_IF_MRF24W) || defined(TCPIP_IF_MRF24WN) 
            WF_INT_IE = 1;
            #endif
...
//                TCPIP_STACK_FREE_FUNC(pHttpCon->uploadBufferStart);
                ...
                if(SYS_FS_Mount(SYS_FS_SD_VOL, LOCAL_WEBSITE_PATH_FS, MPFS2, 0, NULL)  == SYS_FS_RES_SUCCESS)
#elif defined(DRV_SST25_CLIENTS_NUMBER) || defined(DRV_SQI_CLIENTS_NUMBER)       
                if( SYS_FS_Mount("/dev/mtda1", LOCAL_WEBSITE_PATH_FS, MPFS2, 0, NULL) == SYS_FS_RES_SUCCESS )
#else
